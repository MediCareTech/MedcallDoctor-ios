...
-(void)setCellData:(NSArray *)arrData :(NSInteger)index
{
    ...
    if([[NSUserDefaults standardUserDefaults] objectForKey:@"fromPatLookUp"])
        if (![[[NSUserDefaults standardUserDefaults] objectForKey:@"fromPatLookUp"] boolValue])
            self.lblPatName.text = [NSString stringWithFormat:@"Question posted by %@", [[arrData objectAtIndex:index]objectForKey:@"patname"]];
