...
- (void)viewDidLoad
{
    ...
    self.scrolView.contentSize=CGSizeMake(self.scrolView.frame.size.width, self.btnSubmit.frame.origin.y+self.btnSubmit.frame.size.height+100);
    if ([[NSUserDefaults standardUserDefaults]objectForKey:@"HosName"])
    {
...
-(void)makeRequestForCreatdits
{
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@",WB_BASEURL,@"mpack"];//@"mdocs"];
...
-(void) uploadImage:(NSDictionary *)info
{
    ...
    NSData *imageData = UIImagePNGRepresentation(image);
     ...
     NSString *strUrl=[NSString stringWithFormat:@"%@mnewqa",WB_BASEURL];
