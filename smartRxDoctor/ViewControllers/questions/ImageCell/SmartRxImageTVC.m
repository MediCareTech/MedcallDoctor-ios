...
-(void)setImageToData:(NSArray *)arrImages startCount:(NSInteger)imageCount imgType:(BOOL)QA{
    ...
    for (NSInteger i = imageStartCount; i< [arrImages count]; i++, j++) {
        ...
        if (QA) {
            [btn.imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",IMAGE_URL,arrImages[i]]] placeholderImage:[UIImage imageNamed:@"img_placeholder@2x.png"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL){
                if (error) {
                    [btn setBackgroundImage:[UIImage imageNamed:@"img_placeholder@2x.png"] forState:UIControlStateNormal];
                }else
