...
-(void)makeRequestForReplyPost
{
    ...
    {
        dictTemp=[NSDictionary dictionaryWithObjectsAndKeys:sectionId,@"sessionid",[self.dictMsgDetails objectForKeyedSubscript:@"qid"],@"qid",self.txtViewReply.text,@"reply",strSeccionName,@"session_name",@"1",@"close", nil];
    }
    else{
...
//        self.btnPostReply.frame=CGRectMake(self.btnPostReply.frame.origin.x, self.viwRatings.frame.origin.y+self.viwRatings.frame.size.height, self.btnPostReply.frame.size.width, self.btnPostReply.frame.size.height);
        self.scrolView.contentSize=CGSizeMake(self.scrolView.frame.size.width,self.btnPostReply.frame.origin.y+self.btnPostReply.frame.size.height +80);
    }
}
...
-(void) uploadImage:(NSDictionary *)info
{
    ...
    NSString *strUrl=[NSString stringWithFormat:@"%@dreply",WB_BASEURL];
