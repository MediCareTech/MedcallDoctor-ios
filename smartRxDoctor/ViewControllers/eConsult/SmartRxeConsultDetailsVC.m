...
{
    MBProgressHUD *HUD;
    BOOL sessionDidConnect,disapperViewProcess;
    CGFloat viewWidth, viewHeight;
    CGFloat heightLbl;
    CGSize viewSize;
    CGFloat height;
    UILabel *repLabel, *suggLbl;
    UILabel *reqLabel;
    NSString *token;
...
//    self.medicationTable.delegate = self;
    ...
    phoneNumber = [self.dictResponse objectForKey:@"primaryphone"];
...
//    self.suggestionContentLabel.backgroundColor = [UIColor brownColor];
    ...
    sectionTitlesArray = [@[@"Patient Reports",@"Patient Notes/Symptoms", @"Requests", @"Doctor Suggestion/ Prescription"] mutableCopy];
...
//    self.connectBtn.hidden = YES;
   ...
   // NSString *strDatTime=[NSString stringWithFormat:@"%@ %@",self.selectedModel.bookingDate,self.selectedModel.bookingTime];
    ...
    self.eConsultDateTime.text=[NSString timeFormating:strDatTime funcName:@"econsult"];
    //[self.dictResponse objectForKey:@"status"]
    ...
    if ([[self.dictResponse objectForKey:@"symptom"] isKindOfClass:[NSString class]] && [[self.dictResponse objectForKey:@"symptom"] length] > 0)
    {
    ...
    if ([self.dictResponse[@"suggestion"] isKindOfClass:[NSString class]] || [self.arrayReportFiles count] || [self.arrayDoctorSuggestionFiles count])
    {
//        [sectionTitlesArray addObject:@"Doctor Suggestion"];
        self.suggestionContentLabel.text = self.dictResponse[@"suggestion"];
        [self estimatedHeight: self.dictResponse[@"suggestion"]];
    ...
    [[[_ref child:@"heart"] child:[self.dictResponse objectForKey:@"patientid"]] observeEventType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        NSLog(@"fire base data.......:%@",snapshot.value);
        if ([snapshot.value isEqual:[NSNull null]]) {
            return ;
        }
        NSDictionary *heartbeat = snapshot.value;
        NSTimeInterval _interval=[heartbeat[@"timeStamp"] doubleValue];
        
        NSDate *date = [NSDate dateWithTimeIntervalSince1970:_interval];
        NSLog(@"fire base date.......:%@",date);
        
        NSDate *today = [[NSDate alloc] init];
        NSLog(@"%@", today);
        NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
        NSDateComponents *offsetComponents = [[NSDateComponents alloc] init];
        [offsetComponents setMinute:-5]; // note that I'm setting it to -1
        NSDate *finalDate = [gregorian dateByAddingComponents:offsetComponents toDate:today options:0];
        NSLog(@"current date.......:%@",finalDate);
        
        NSComparisonResult result = [date compare:finalDate];
        ...
        self.heartBeatLbl.text = [NSString stringWithFormat:@"%@",heartbeat[@"heartbeat"]];
...
//    [self.scrollView scrollRectToVisible:CGRectMake(viewWidth*4,0, viewWidth, 800) animated:NO];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"modifyEconsult"])
    {
        ((SmartRxeConsultModify *)segue.destinationViewController).dictResponse = [[NSMutableDictionary alloc] init];
        ((SmartRxeConsultModify *)segue.destinationViewController).dictResponse = sender;
        
        if ([self.dictResponse[@"apptype"] integerValue] == 3) {
            ((SmartRxeConsultModify *)segue.destinationViewController).scheduleType = @"3";
...
{
    [self makeRequestToConfirm];
}

- (IBAction)modifyBtnClicked:(id)sender
{
    disapperViewProcess = NO;
    [self performSegueWithIdentifier:@"modifyEconsult" sender:self.dictResponse];
}
...
- (IBAction)reportsAddClicked:(id)sender //Suggestion or prescription adding
{
...
- (void)makeRequestToPushEconsultNotification
{
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@",WB_BASEURL,@"dinit"];
...
- (void)makeRequestToGetToken
{
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@",WB_BASEURL,@"detoken"];
    [[SmartRxCommonClass sharedManager] postOrGetData:url postPar:bodyText method:@"POST" setHeader:NO  successHandler:^(id response) {
...
- (void)makeRequestToAddReports
{
    ...
    NSDictionary *dictTemp=[NSDictionary dictionaryWithObjectsAndKeys:sectionId,@"sessionid",[self.dictResponse objectForKey:@"conid"],@"econid",strSeccionName,@"session_name",[self.dictResponse objectForKey:@"patientid"],@"patid",@"2",@"apptype", nil];
    if (!isPdfSelected) {
...
- (void)makeRequestToAddRequest:(NSString *)requestText
{
    ...
    if (requestText.length)
    {
        ...
        NSString *url=[NSString stringWithFormat:@"%@%@",WB_BASEURL,@"dereqadd"];
        [[SmartRxCommonClass sharedManager] postOrGetData:url postPar:bodyText method:@"POST" setHeader:NO  successHandler:^(id response) {
            NSLog(@"sucess 1 %@",response);
            if ([[response objectForKey:@"authorized"]integerValue] == 0 && [[response objectForKey:@"result"]integerValue] == 0)
            {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if ([[response objectForKey:@"addreq"] integerValue] == 1)
                    {
                        if ([self.arrayRequestData count] >= 2)
                            [self.requestContentTable setContentSize:CGSizeMake(self.requestContentTable.contentSize.width, self.requestContentTable.contentSize.height+ 50 - [self tableView:self.requestContentTable heightForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]])];
                        [self makeRequestForRequests];
                    }
                    else
                    {
...
- (void)makeRequestToAddSymptoms:(NSString *)symptomText
{
    ...
    [self addSpinnerView];
    NSString *sectionId=[[NSUserDefaults standardUserDefaults]objectForKey:@"sessionid"];
    //NSString *bodyText = [NSString stringWithFormat:@"%@=%@&conid=%@&suggestion=%@",@"sessionid",sectionId,[self.dictResponse objectForKey:@"conid"],symptomText];
    NSString *bodyText = [NSString stringWithFormat:@"%@=%@&conid=%@&suggestion=%@&apptype=2&patid=%@",@"sessionid",sectionId,[self.dictResponse objectForKey:@"conid"],symptomText,self.dictResponse[@"patientid"]];
...
//                    NSLog(@"The height is : %f", heightLbl);
                    self.suggestionContentLabel.frame = CGRectMake(self.suggestionContentLabel.frame.origin.x, self.suggestionContentLabel.frame.origin.y, self.suggestionViewEdit.frame.size.width-20, heightLbl+self.suggestionContentLabel.frame.size.height);
                    ...
                    self.suggestionContentTable.frame = CGRectMake(self.suggestionContentTable.frame.origin.x, self.suggestionContentLabel.frame.origin.y + self.suggestionContentLabel.frame.size.height+10, self.suggestionContentTable.frame.size.width, tableHeight);
                    ...
                    self.suggestionViewEdit.frame = CGRectMake(self.suggestionViewEdit.frame.origin.x, self.suggestionViewEdit.frame.origin.y, self.suggestionViewEdit.frame.size.width , self.suggestionViewEdit.frame.size.height+tableHeight);
        ...
        if ([[response objectForKey:@"authorized"]integerValue] == 0 && [[response objectForKey:@"result"]integerValue] == 0)
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([self.arrayReportFiles count])
                {
                    ...
                    [self.reportScroll setContentSize:CGSizeMake(self.scrollView.frame.size.width, tableHeight+200)];
//                    self.reportViewEdit.frame = CGRectMake(self.reportViewEdit.frame.origin.x, self.reportViewEdit.frame.origin.y, self.reportViewEdit.frame.size.width, self.reportViewEdit.frame.size.height+tableHeight);
                }
                else
                {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([self.arrayDoctorSuggestionFiles count])
                    {
                        if (self.suggestionContentLabel.text == nil)
...
//                        [self.suggestionScroll setContentSize:CGSizeMake(self.scrollView.frame.size.width, self.suggestionScroll.frame.size.height+tableHeight)];
                    ...
                    else
                    {
                        if ([self.suggestionContentLabel.text length] == 0 || self.suggestionContentLabel.text == nil)
                            self.suggestionContentLabel.text = nil;
...
- (void)makeRequestForRequests
{
    ...
    NSString *sectionId=[[NSUserDefaults standardUserDefaults]objectForKey:@"sessionid"];
    NSString *bodyText = [NSString stringWithFormat:@"%@=%@&conid=%@&patid=%@",@"sessionid",sectionId,[self.dictResponse objectForKey:@"conid"],[self.dictResponse objectForKey:@"patientid"]];
    NSString *url=[NSString stringWithFormat:@"%@%@",WB_BASEURL,@"dereqlist"];
    [[SmartRxCommonClass sharedManager] postOrGetData:url postPar:bodyText method:@"POST" setHeader:NO  successHandler:^(id response) {
        NSLog(@"sucess 1 %@",response);
        if ([[response objectForKey:@"authorized"]integerValue] == 0 && [[response objectForKey:@"result"]integerValue] == 0)
        {
...
//                    self.requestContentTable.frame = CGRectMake(0, self.requestContentTable.frame.origin.y, viewWidth, self.requestContentTable.frame.size.height-57);
                    [self.requestContentTable setContentSize:CGSizeMake(self.requestContentTable.contentSize.width, self.requestContentTable.contentSize.height + [self tableView:self.requestContentTable heightForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]])];
...
- (void)makeRequestToConfirm
{
    ...
    bodyText = [NSString stringWithFormat:@"%@=%@&type=%d&econ_date=%@&econ_doctor=%@&patid=%@&econ_id=%@&econ_status=%d&econ_specialty=%@&econ_time=%@&econ_method=%d&econ_cancel_reason=%@",@"sessionid",sectionId, 2, [self.dictResponse objectForKey:@"appdate"],[[NSUserDefaults standardUserDefaults] objectForKey:@"userid"], [self.dictResponse objectForKey:@"patientid"], [self.dictResponse objectForKey:@"conid"], 2, [self.dictResponse objectForKey:@"specality"], [NSString timeFormating:strDatTime funcName:@"appointment"], [[self.dictResponse objectForKey:@"app_method"] integerValue], @"" ];
...
-(void) uploadImage:(NSDictionary *)info
{
    ...
    UIImage *image = newImage;//[info valueForKey:UIImagePickerControllerOriginalImage];
...
//    if ([[NSUserDefaults standardUserDefaults]boolForKey:@"RingEconsult"] == YES)
//    {
//        [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"RingEconsult"];
//        [[NSUserDefaults standardUserDefaults]synchronize];
//        if ([self.videoConsultView isHidden] && [self.connectBtn.currentTitle isEqualToString:@"      Connect"])
//        {
//            NSError *error;
...
- (void)videoDisabled:(NSString *)imgName
{
